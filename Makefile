...
.PHONY: build test race cover cover-check fuzz-smoke bench-smoke clean format-check lint vuln readme-check release-notes-check api-check output-contract-check portfolio-check dns-snapshot-check auth-record-check provider-catalog-check dns-health-check report-evidence-check ci
...
bench-smoke:
	go test -run=^$$ -bench='BenchmarkLoadBytes|BenchmarkSummary|BenchmarkUnauthenticatedSources|BenchmarkNormalizePortfolio|BenchmarkCollectDNSSnapshotSharedPortfolio|BenchmarkParseAuthenticationRecords|BenchmarkEvaluateDNSHealthLargePortfolio|BenchmarkAnalyzeReportEvidence' -benchtime=1x ./...

ci: format-check mod-verify mod-verify-local lint vuln readme-check release-notes-check api-check output-contract-check portfolio-check dns-snapshot-check auth-record-check provider-catalog-check dns-health-check report-evidence-check test race cover-check fuzz-smoke bench-smoke build
